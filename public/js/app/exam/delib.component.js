...
angular.module('exam')
        .component('delibConfig',{
            templateUrl: 'deliberation',
            controller: delibCtrl 
...
function delibCtrl($timeout,$http,$location,$mdDialog,$scope,DTOptionsBuilder,DTColumnDefBuilder){
    var $ctrl = this;
    $ctrl.grades = [];
    ...
    $timeout(
     $http.get('delibConfig').then(function(response){
         $ctrl.conditions = response.data[0];
                for(var i=0;i<$ctrl.conditions.length;i++)
                {
                    $ctrl.conditions[i].num = i+1;
...
$ctrl.formatDate = function(date){
  ...
  $ctrl.redirect= function(id){
     ...
     $location.path("/newDeliberation/"+id);
