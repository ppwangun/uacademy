...
angular.module('exam')
        .component('newdelibConfig',{
            templateUrl: 'delibConditions',
            controller: newdelibCtrl 
});
function newdelibCtrl($timeout,$http,$location,$mdDialog,$routeParams,$scope,toastr,DTOptionsBuilder,DTColumnDefBuilder){
    var id,exam_id;
    var $ctrl= this;
 ...
 var condition_id =$routeParams.id;
     ...
     $timeout(    
         ...
         if(condition_id)
         {
            ...
            headers : {'Accept' : 'application/json'}
             ...
             $http.get('delibConfig',config).then(
             function successCallback(response){
                $ctrl.delibName = response.data[0].label;
                $ctrl.delibCondition = response.data[0].delibCondition;
             },
             ...
             $http.get('classesAssociatedToDelib',config).then(
             function successCallback(response){
...
$ctrl.updateDelib = function(){
        var data ={
            delibName : $ctrl.delibName,
            classes : $ctrl.classes,
            condition : $ctrl.delibCondition,
            id : condition_id
        };
        var config = {
        params: data,
        headers : {'Accept' : 'application/json'}
          ...
          $timeout(
              $http.put('delibConfig',data,config).then(function(response){
              toastr.success("Opéraction effectuée avec succès")

          }),500);    
};

$ctrl.deleteDelib= function(ev){
      var data = {id: condition_id}; 
      var config = {
      params: data,
      headers : {'Accept' : 'application/json'}
...
//open de confirm window
    $mdDialog.show(confirm).then(function() {
        ...
        $http.delete('delibConfig',config).then(
          function successCallback(response){
              toastr.success("Opéraction effectuée avec succès");
              $location.path("/deliberation") ; 
...
$ctrl.newDelib = function(){
    var delibParams= {};
    delibParams.delibName = $ctrl.delibName;
    delibParams.cycle = $ctrl.selectedCycle;
    delibParams.classes = $ctrl.classes;
    delibParams.condition = $ctrl.delibCondition;
    
    $timeout(
        $http.post('delibConfig',delibParams).then(
        function successCallback(response){
        toastr.success("Opéraction effectuée avec succès");
        $ctrl.isUpdate = true;
...
$ctrl.addDelibCondition = function(){
        ...
        $ctrl.grade.grade_id =grade_id;
          ...
          $timeout(
              $http.post('delibConfig',$ctrl.delibCondition).then(function(response){
             toastr.success("Opéraction effectuée avec succès");
...
//open de confirm window
    ...
    $ctrl.showCreateRangeOfValues= function(ev){
        ...
        $mdDialog.show({
          ...
          templateUrl: 'js/app/exam/delibRange.html',
